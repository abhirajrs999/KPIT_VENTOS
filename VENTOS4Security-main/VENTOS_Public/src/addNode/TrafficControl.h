...
private:
    ...
    std::map<uint32_t, fdiEntry_t> allFdi;
...
private:
    ...
    void parseFdi(rapidxml::xml_node<> *);
    void checkFdiConflicts(fdiEntry_t &, uint32_t);
    uint32_t checkFdiConflicts_begin(fdiEntry_t &, uint32_t);
    uint32_t checkFdiConflicts_edgeId(fdiEntry_t &, uint32_t);
    uint32_t checkFdiConflicts_laneId(fdiEntry_t &, uint32_t);
    void controlFdi();
    void controlFdi_begin(fdiEntry_t &);
    void controlFdi_edgeId(fdiEntry_t &);
    void controlFdi_laneId(fdiEntry_t &);
    void DoFalsificationAttack(BeaconVehicle * wsm, double currentFdi, fdiEntry_t &entry);

    void readFile(fdiEntry_t &);
    void processFileTokens(std::vector<std::string> &, uint32_t &, fdiEntry_t &fdiEntry);
    void verifyFile(fdiEntry_t &);
