...
void TrafficControl::readInsertion(std::string addNodePath)
{
    ...
    if(allSpeed.empty() && allFdi.empty() && allOptSize.empty() && allPltMerge.empty() && allPltSplit.empty() && allPltLeave.empty()) //&& allFdi.empty())
...
void TrafficControl::verifyFile(speedEntry_t &speedEntry)
{
    ...
    double falsedata = speedEntry.fileContent[0].speed;

    if(falsedata == 0)
    {

        // we have only one column denoting speed
        if(speedEntry.fileContent[1].time == -1)
        {
            double prevSpeed = speedEntry.fileContent[1].speed;

            // find the max acceleration/deceleration needed to follow the file
            for(unsigned int i = 2; i < speedEntry.fileContent.size(); i++)
            {
    ...
    else
    {
        ...
        {
            double falseaccel = speedEntry.fileContent[i].speed;
                
                std::cout << "falsedata : " << i << " = " << falseaccel ;
                std::cout << "\n" << std::flush;
            }
        }
...
void TrafficControl::parseFdi(rapidxml::xml_node<> *pNode)
{
    uint32_t nodeCount = 1;
    ...
    // Iterate over all 'speed' nodes
    for(rapidxml::xml_node<> *cNode = pNode->first_node(fdi_tag.c_str()); cNode; cNode = cNode->next_sibling())
    {
        if(std::string(cNode->name()) != fdi_tag)
            continue;

        std::vector<std::string> validAttr = {"id", "begin", "end", "duration", "edgeId", "edgePos", "laneId",
                "lanePos", "value", "maxAccel", "maxDecel", "file", "headerLines", "columns", "from", "to"};
        xmlUtil::validityCheck(cNode, validAttr);
        ...
        uint32_t headerLines = xmlUtil::getAttrValue_int(cNode, "headerLines", false, 0);
        std::string columns_str = xmlUtil::getAttrValue_string(cNode, "columns", false, "");
        std::string from_str = xmlUtil::getAttrValue_string(cNode, "from");
        std::string to_str = xmlUtil::getAttrValue_string(cNode, "to");

        if(begin != -1 && begin < 0)
            throw omnetpp::cRuntimeError("attribute 'begin' cannot be negative in element '%s'", fdi_tag.c_str());

        if(end != -1 && begin != -1 && end <= begin)
            throw omnetpp::cRuntimeError("attribute 'end' cannot be lower than 'begin' in element '%s'", fdi_tag.c_str());

        if(duration != -1 && duration <= 0)
            throw omnetpp::cRuntimeError("attribute 'duration' cannot be negative in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("end") && cNode->first_attribute("duration"))
            throw omnetpp::cRuntimeError("attribute 'duration' and 'end' cannot be present together in element '%s'", fdi_tag.c_str());

        if(maxAccel != -1 && maxAccel < 0)
            throw omnetpp::cRuntimeError("attribute 'maxAccel' cannot be negative in element '%s'", fdi_tag.c_str());

        if(maxDecel != -1 && maxDecel < 0)
            throw omnetpp::cRuntimeError("attribute 'maxDecel' cannot be negative in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("edgeId") && (cNode->first_attribute("laneId") || cNode->first_attribute("begin")))
            throw omnetpp::cRuntimeError("attribute 'laneId/begin' is redundant when 'edgeId' is present in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("laneId") && (cNode->first_attribute("edgeId") || cNode->first_attribute("begin")))
            throw omnetpp::cRuntimeError("attribute 'edgeId/begin' is redundant when 'laneId' is present in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("begin") && (cNode->first_attribute("edgeId") || cNode->first_attribute("laneId")))
            throw omnetpp::cRuntimeError("attribute 'edgeId/laneId' is redundant when 'begin' is present in element '%s'", fdi_tag.c_str());

        if(!cNode->first_attribute("begin") && !cNode->first_attribute("edgeId") && !cNode->first_attribute("laneId"))
            throw omnetpp::cRuntimeError("One of 'begin', 'edgeId' or 'laneId' attributes should be defined in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("edgePos") && !cNode->first_attribute("edgeId"))
            throw omnetpp::cRuntimeError("attribute 'edgePos' requires 'edgeId' in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("lanePos") && !cNode->first_attribute("laneId"))
            throw omnetpp::cRuntimeError("attribute 'lanePos' requires 'laneId' in element '%s'", fdi_tag.c_str());

        if(cNode->first_attribute("file") && cNode->first_attribute("value"))
            throw omnetpp::cRuntimeError("attribute 'file' and 'value' cannot be present at the same time in element '%s'", fdi_tag.c_str());

        if(headerLines < 0)
            throw omnetpp::cRuntimeError("attribute 'headerLine' is invalid in element '%s'", fdi_tag.c_str());
        ...
        if(value_str != "")
        {
            ...
            catch(...)
            {
                // this is to check the conflicts more easily
                if(!cNode->first_attribute("end") && !cNode->first_attribute("duration"))
                    throw omnetpp::cRuntimeError("attribute 'end' or 'duration' is mandatory when fdi is a function in element '%s'", speed_tag.c_str());
            }
        }
        ...
        if(edgeId_str != "")
        {
            auto ii = std::find(allEdges.begin(), allEdges.end(), edgeId_str);
            if(ii == allEdges.end())
                throw omnetpp::cRuntimeError("Edge '%s' is not a valid edge in element '%s'", edgeId_str.c_str(), fdi_tag.c_str());

            // edgePos cannot be negative
            if(edgePos < 0)
                throw omnetpp::cRuntimeError("attribute 'edgePos' cannot be negative in element '%s'", fdi_tag.c_str());
            ...
            if(edgePos > maxLaneLength)
                throw omnetpp::cRuntimeError("attribute 'edgePos' is bigger that the maximum lane length in element '%s'", fdi_tag.c_str());
        }
        ...
        if(laneId_str != "")
        {
            auto ii = std::find(allLanes.begin(), allLanes.end(), laneId_str);
            if(ii == allLanes.end())
                throw omnetpp::cRuntimeError("Lane '%s' is not a valid lane in element '%s'", laneId_str.c_str(), fdi_tag.c_str());

            double laneLength = TraCI->laneGetLength(laneId_str);

            if(lanePos > laneLength)
                throw omnetpp::cRuntimeError("attribute 'lanePos' is bigger than the lane length in element '%s'", fdi_tag.c_str());

            if(lanePos < 0)
            {
                lanePos += laneLength;
                if(lanePos < 0)
                    throw omnetpp::cRuntimeError("attribute 'lanePos' is not valid in element '%s'", fdi_tag.c_str());
            }
        }

        auto it = allFdi.find(nodeCount);
        if(it == allFdi.end())
        {
            fdiEntry_t entry = {};
            ...
            // check for conflicts before adding
            checkFdiConflicts(entry, nodeCount);

            // read the external file line by line if present
            readFile(entry);

            allFdi.insert(std::make_pair(nodeCount, std::move(entry)));
        }
        else
            throw omnetpp::cRuntimeError("Multiple %s with the same 'id' %s is not allowed!", fdi_tag.c_str(), id_str.c_str());

        // set symbol_table and expression
        auto ii = allFdi.find(nodeCount);
        ii->second.symbol_table.add_variable("t", ii->second.expressionVariable);
        ii->second.expression.register_symbol_table(ii->second.symbol_table);
...
void TrafficControl::readFile(fdiEntry_t &fdiEntry)
{
    if(fdiEntry.file_str == "")
        return;

    if(isCSV(fdiEntry.file_str))
    {
        std::vector<std::vector<std::string>> result;
        CSV2TXT(fdiEntry.file_str, result);

        uint32_t rowCount = fdiEntry.headerLines + 1;
        for(unsigned int k = fdiEntry.headerLines; k < result.size(); ++k)
        {
            processFileTokens(result[k], rowCount, fdiEntry);
    ...
    {
        // open file for reading
        std::ifstream ifile(fdiEntry.file_str);
        if(!ifile.is_open())
            throw omnetpp::cRuntimeError("Cannot open file '%s' in element '%s'", fdiEntry.file_str.c_str(), fdi_tag.c_str());
        ...
        while (std::getline(ifile, str))
        {
            if(rowCount <= (uint32_t)fdiEntry.headerLines)
            {
                rowCount++;
                continue;
            }
            ...
            processFileTokens(tokens, rowCount, fdiEntry);

            rowCount++;
        }
    }

    if(fdiEntry.fileContent.empty())
        LOG_WARNING << boost::format("\nWARNING: External file '%s' is empty \n") % fdiEntry.file_str << std::flush;

    // at least two elements exists
    if(fdiEntry.fileContent.size() > 1)
        verifyFile(fdiEntry);
}


void TrafficControl::processFileTokens(std::vector<std::string> &tokens, uint32_t &rowCount, fdiEntry_t &fdiEntry)
{
    // skip the empty line
    if(tokens.empty())
        return;
    ...
    else if(tokens.size() == 1)
    {
        try
        {
            ...
            format_t entry;
            entry.fdi = boost::lexical_cast<double>(tokens[0]);

            // get the last inserted element and make sure it has the same format as mine
            if(!fdiEntry.fileContent.empty() && fdiEntry.fileContent.back().time != -1)
                throw omnetpp::cRuntimeError("External file '%s' is not formatted correctly", fdiEntry.file_str.c_str());

            fdiEntry.fileContent.push_back(entry);
        }
        catch(...)
        {
            throw omnetpp::cRuntimeError("Cannot convert value '%s' to double located in row '%d' of external file '%s'!",
    ...
    // we have two columns denoting the time and speed
    else if(tokens.size() == 2 && fdiEntry.columns == "")
    {
        try
        {
            boost::trim(tokens[0]);
            ...
            format_t entry;
            entry.time = boost::lexical_cast<double>(tokens[0]);
            entry.fdi = boost::lexical_cast<double>(tokens[1]);
    ...
    else if(fdiEntry.columns != "")
    {
        std::vector<std::string> columns_num;
        boost::split(columns_num, fdiEntry.columns, boost::is_any_of(","));
        ...
        else if(columns_num.size() == 1)
        {
            ...
            {
                boost::trim(columns_num[0]);
                column_fdi = boost::lexical_cast<int>(columns_num[0]);
            }
            catch(...)
            {
                throw omnetpp::cRuntimeError("Cannot convert value '%s' to int in column attribute '%s'",
                        columns_num[0].c_str(),
                        fdiEntry.columns.c_str());
            }

            if(column_fdi <= 0 || column_fdi > (int)tokens.size())
                throw omnetpp::cRuntimeError("Attribute 'columns' is invalid: %d", column_fdi);
            ...
            try
            {
                boost::trim(tokens[column_fdi]);

                format_t entry;
                entry.fdi = boost::lexical_cast<double>(tokens[column_fdi]);
        ...
        else if(columns_num.size() == 2)
        {
            ...
            try
            {
                ...
                column_time = boost::lexical_cast<int>(columns_num[0]);
                column_fdi = boost::lexical_cast<int>(columns_num[1]);
            }
            catch(...)
            {
                throw omnetpp::cRuntimeError("Cannot convert value to int in column attribute '%s'", fdiEntry.columns.c_str());
            }
        ...
        throw omnetpp::cRuntimeError("External file '%s' has multiple columns and I don't know which columns to use. Forgot to use the 'columns' attribute?", fdiEntry.file_str.c_str());
}


void TrafficControl::verifyFile(fdiEntry_t &fdiEntry)
{
    double maxAccel = -std::numeric_limits<double>::max();
    double maxDecel = std::numeric_limits<double>::max();
    double falsedata = fdiEntry.fileContent[0].fdi;

    if(falsedata == 0)
    {

        // we have only one column denoting speed
        if(fdiEntry.fileContent[1].time == -1)
        {
            double prevFdi = fdiEntry.fileContent[1].fdi;

            // find the max acceleration/deceleration needed to follow the file
            for(unsigned int i = 2; i < fdiEntry.fileContent.size(); i++)
            {
                double currentFdi = fdiEntry.fileContent[i].fdi;

                double accel = (currentFdi - prevFdi) / SUMO_timeStep;
                if(accel >= 0)
                    maxAccel = std::max(maxAccel, accel);
                else
                    maxDecel = std::min(maxDecel, accel);
        ...
        {
            double prevFdi = fdiEntry.fileContent[1].fdi;
            double prevTime = fdiEntry.fileContent[0].time;

            // find the max acceleration/deceleration needed to follow the file
            for(unsigned int i = 2; i < fdiEntry.fileContent.size(); i++)
            {
                double currentTime = fdiEntry.fileContent[i].time;
                double currentFdi = fdiEntry.fileContent[i].fdi;

                // make sure times are in ascending order
                if(currentTime < prevTime)
                    throw omnetpp::cRuntimeError("The time column in file '%s' is not in ascending order", fdiEntry.file_str.c_str());

                double accel = (currentFdi - prevFdi) / (currentTime - prevTime);
                if(accel >= 0)
                    maxAccel = std::max(maxAccel, accel);
                else
                    maxDecel = std::min(maxDecel, accel);
    ...
    else
    {
        ...
        for(unsigned int i = 1; i < fdiEntry.fileContent.size(); i++)
        {
            double falseaccel = fdiEntry.fileContent[i].fdi;

                
                std::cout << "falsedata new field : " << i << " = " << falseaccel ;
                std::cout << "\n" << std::flush;
            }
            std::cout << "from id : " << fdiEntry.from_str << ", to id : " << fdiEntry.to_str ;
            std::cout << "\n" << std::flush;
        }

    if(maxAccel != -std::numeric_limits<double>::max())
    {
        LOG_WARNING << boost::format("\nFdi change of vehicle '%s' using file '%s' requires maximum acceleration of '%d' \n") %
                fdiEntry.id_str %
                fdiEntry.file_str %
                maxAccel << std::flush;
    }
    ...
    if(maxDecel != std::numeric_limits<double>::max())
    {
        LOG_WARNING << boost::format("\nFdi change of vehicle '%s' using file '%s' requires maximum deceleration of '%d' \n") %
                fdiEntry.id_str %
                fdiEntry.file_str %
                maxDecel << std::flush;
    }
...
{
    // Commented by Rohit
    //std::cout << "Called handle message function hurrah" ;
    //std::cout << "\n" << std::flush;
    ...
    if (msg->getKind() == TYPE_BEACON_VEHICLE)
    {
        ...
        for(auto &entry : allFdi)
        {
            for(unsigned int i = 2; i < entry.second.fileContent.size(); i++)
            {
                double currentFdi = entry.second.fileContent[i].fdi;
                DoFalsificationAttack(wsm, currentFdi, entry.second);
            }
        }
    }
...
void TrafficControl::DoFalsificationAttack(BeaconVehicle* wsm, double currentFdi, fdiEntry_t &entry)
{
    ...
    //FalseMsg->setAccel(9);
    int depthofvehicle = entry.from_str[4]-'0';
    if(wsm->getPlatoonDepth() == depthofvehicle) //wsm->getSender() != "veh.2" , find some way to get vehicle ids
    {
        double falsevalue = currentFdi + FalseMsg->getAccel();
        FalseMsg->setAccel(falsevalue); //6.
       // RGB newColor = Color::colorNameToRGB("red");
       // TraCI->vehicleSetColor(entry.to_str, newColor);
        //EV << "Rohit, " << wsm->getSender() << std::endl;
    }
...
void TrafficControl::checkFdiConflicts(fdiEntry_t &fdiEntry, uint32_t nodeCount)
{
    if(fdiEntry.begin != -1)
    {
        uint32_t conflictedFdiNode = checkFdiConflicts_begin(fdiEntry, nodeCount);
        if(conflictedFdiNode != 0)
            throw omnetpp::cRuntimeError("Fdi node '%d' and '%d' in TrafficControl with id '%s' have conflicts. "
                    "They are both trying to change the fdi of vehicle '%s' at time '%f'", conflictedFdiNode,
                    nodeCount,
                    this->id.c_str(),
                    fdiEntry.id_str.c_str(),
                    fdiEntry.begin);
    }
    else if(fdiEntry.edgeId_str != "")
    {
        uint32_t conflictedFdiNode = checkFdiConflicts_edgeId(fdiEntry, nodeCount);
        if(conflictedFdiNode != 0)
            throw omnetpp::cRuntimeError("Fdi node '%d' and '%d' in TrafficControl with id '%s' have conflicts. "
                    "They are both trying to change the fdi of vehicle '%s' at the same location", conflictedFdiNode,
                    nodeCount,
                    this->id.c_str(),
                    fdiEntry.id_str.c_str());
    }
    else if(fdiEntry.laneId_str != "")
    {
        uint32_t conflictedFdiNode = checkFdiConflicts_laneId(fdiEntry, nodeCount);
        if(conflictedFdiNode != 0)
            throw omnetpp::cRuntimeError("Fdi node '%d' and '%d' in TrafficControl with id '%s' have conflicts. "
                    "They are both trying to change the fdi of vehicle '%s' at the same location", conflictedFdiNode,
                    nodeCount,
...
uint32_t TrafficControl::checkFdiConflicts_begin(fdiEntry_t &fdiEntry, uint32_t nodeCount)
{
    typedef struct fdiChangeEntry
    {
        uint32_t nodeCount;
        double fromTime;
        double toTime;
    ...
    static std::map<std::string /*vehID*/, std::vector<fdiChangeEntry_t>> allFdiChange;

    double fromTime = fdiEntry.begin;
    double toTime = -1;
    if(fdiEntry.end != -1)
        toTime = fdiEntry.end;
    else if(fdiEntry.duration != -1)
        toTime = fromTime + fdiEntry.duration;
    else if(fdiEntry.end == -1 && fdiEntry.duration == -1)
        toTime = fromTime;

    ASSERT(fromTime != -1);
    ASSERT(toTime != -1);

    auto ii = allFdiChange.find(fdiEntry.id_str);
    // this is the first fdi control for this vehicle
    if(ii == allFdiChange.end())
    {
        fdiChangeEntry_t entry = {nodeCount, fromTime, toTime};
        std::vector<fdiChangeEntry_t> entry2 = {entry};
        allFdiChange[fdiEntry.id_str] = entry2;
    ...
    {
        // for each node on this lane
        for(auto &fdiNode : ii->second)
        {
            // check for overlap
            if((toTime >= fdiNode.fromTime && toTime <= fdiNode.toTime) ||
                    (fromTime >= fdiNode.fromTime && fromTime <= fdiNode.toTime))
            {
                return fdiNode.nodeCount;
            }
        }
...
uint32_t TrafficControl::checkFdiConflicts_edgeId(fdiEntry_t &fdiEntry, uint32_t nodeCount)
{
    typedef struct fdiChangeEntry
    {
        uint32_t nodeCount;
    ...
    if(ii == allFdiChange.end())
    {
        fdiChangeEntry_t entry = {nodeCount, fdiEntry.edgeId_str, fdiEntry.edgePos};
        std::vector<fdiChangeEntry_t> entry2 = {entry};
        allFdiChange[fdiEntry.id_str] = entry2;
    ...
    else
    {
        ...
        {
            // check for overlap
            if(fdiEntry.edgeId_str == fdiNode.edgeId && fdiEntry.edgePos == fdiNode.edgePos)
            {
                return fdiNode.nodeCount;
            }
        }
...
uint32_t TrafficControl::checkFdiConflicts_laneId(fdiEntry_t &fdiEntry, uint32_t nodeCount)
{
    typedef struct fdiChangeEntry
    {
        uint32_t nodeCount;
    ...
    if(ii == allFdiChange.end())
    {
        fdiChangeEntry_t entry = {nodeCount, fdiEntry.laneId_str, fdiEntry.lanePos};
        std::vector<fdiChangeEntry_t> entry2 = {entry};
        allFdiChange[fdiEntry.id_str] = entry2;
    ...
    else
    {
        ...
        {
            // check for overlap
            if(fdiEntry.laneId_str == fdiNode.laneId && fdiEntry.lanePos == fdiNode.lanePos)
            {
                return fdiNode.nodeCount;
            }
        }
...
void TrafficControl::controlFdi()
{
    for(auto &entry : allFdi)
    {
        if(!entry.second.processingEnded)
        {
            ...
            if(entry.second.begin != -1)
            {
                // wait until 'begin'
                if(entry.second.begin <= omnetpp::simTime().dbl())
                    controlFdi_begin(entry.second);
            }
            // edgeId attribute is present
            else if(entry.second.edgeId_str != "")
                controlFdi_edgeId(entry.second);
            // laneId attribute is present
            else if(entry.second.laneId_str != "")
                controlFdi_laneId(entry.second);
        }
    }
}


void TrafficControl::controlFdi_begin(fdiEntry_t &fdiEntry)
{
    ASSERT(fdiEntry.begin != -1);

    // Continuously check if the vehicle is in the network
    auto ii = TraCI->departureArrival.find(fdiEntry.id_str);
    if(ii == TraCI->departureArrival.end())
    {
        LOG_WARNING << boost::format("\nWARNING: Changing the fdi of '%s' at time '%f' is not possible ") % fdiEntry.id_str % fdiEntry.begin;
        LOG_WARNING << boost::format("(it hasn't departed yet) \n") << std::flush;
        fdiEntry.processingEnded = true;
        return;
    }
    else if(ii->second.arrival != -1)
    {
        if(!fdiEntry.processingStarted)
        {
            LOG_WARNING << boost::format("\nWARNING: Changing the fdi of '%s' at time '%f' is not possible ") % fdiEntry.id_str % fdiEntry.begin;
            LOG_WARNING << boost::format("(it left the network at time '%f') \n") % ii->second.arrival << std::flush;
            fdiEntry.processingEnded = true;
    ...
    {
        // save current speed for future
        fdiEntry.oldfdi = TraCI->vehicleGetSpeed(fdiEntry.id_str);

        if(fdiEntry.maxAccel != -1)
            TraCI->vehicleSetMaxAccel(fdiEntry.id_str, fdiEntry.maxAccel);

        if(fdiEntry.maxDecel != -1)
            TraCI->vehicleSetMaxDecel(fdiEntry.id_str, fdiEntry.maxDecel);

        if(fdiEntry.value_str != "")
        {
            try
            {
                boost::trim(fdiEntry.value_str);
                double fdiValue = boost::lexical_cast<double>(fdiEntry.value_str);

                // make sure the speed is not negative
                ASSERT(fdiValue >= 0);

                // set vehicle's speed
                TraCI->vehicleSetSpeed(fdiEntry.id_str, fdiValue);
            }
            catch(...)
            {
                // we cannot convert speed_str to a double!
                // probably it is an arithmetic expression
                // set the flag
                fdiEntry.expressionEvaluationRequired = true;
            }
        }

        fdiEntry.processingStarted = true;
    }

    if(fdiEntry.expressionEvaluationRequired)
    {
        parser_t parser;
        parser.compile(fdiEntry.value_str, fdiEntry.expression);

        // evaluate the speed function at current point
        fdiEntry.expressionVariable = omnetpp::simTime().dbl();
        double functionVal = fdiEntry.expression.value();

        // make sure the speed is not negative
        functionVal = std::max(0., functionVal);

        // set the speed
        TraCI->vehicleSetSpeed(fdiEntry.id_str, functionVal);
    }

    if(fdiEntry.file_str != "" && !fdiEntry.endOfFile)
    {
        if(!fdiEntry.fileContent.empty())
        {
            // we have only speed data
            if(fdiEntry.fileContent[0].time == -1)
            {
                static uint32_t row = 0;

                if(row < fdiEntry.fileContent.size())
                {
                    // set the speed
                    TraCI->vehicleSetSpeed(fdiEntry.id_str, fdiEntry.fileContent[row].fdi);
                    row++;
                }
                else
                {
                    // we reached the end of file
                    fdiEntry.endOfFile = true;
                }
            }
            // we have time-speed data
            else
            {
                ...
                for(unsigned int k = row; k < fdiEntry.fileContent.size(); ++k)
                {
                    if(fdiEntry.fileContent[k].time <= omnetpp::simTime().dbl())
                    {
                        TraCI->vehicleSetSpeed(fdiEntry.id_str, fdiEntry.fileContent[k].fdi);
                        row = k + 1;
                    }
                    else
                    {
    ...
    // if neither 'end' or 'duration' is specified
    if(fdiEntry.end == -1 && fdiEntry.duration == -1)
    {
        // if the 'file' attribute does not exist
        if(fdiEntry.file_str == "")
            fdiEntry.processingEnded = true;
        // if the 'file' attribute exists then end only when endOfFile is set
        else if(fdiEntry.endOfFile)
            fdiEntry.processingEnded = true;
    }
    // 'end' is specified
    else if(fdiEntry.end != -1)
    {
        if(fdiEntry.end <= omnetpp::simTime().dbl())
        {
            // revert the vehicle's old speed
            TraCI->vehicleSetSpeed(fdiEntry.id_str, fdiEntry.oldfdi);
            fdiEntry.processingEnded = true;
        }
    }
    // 'duration' is specified
    ...
    {
        if(fdiEntry.begin + fdiEntry.duration <= omnetpp::simTime().dbl())
        {
            // revert the vehicle's old speed
            TraCI->vehicleSetSpeed(fdiEntry.id_str, fdiEntry.oldfdi);
            fdiEntry.processingEnded = true;
...
void TrafficControl::controlFdi_edgeId(fdiEntry_t &fdiEntry)
{
    // Continuously check if the vehicle is in the network
    auto ii = TraCI->departureArrival.find(fdiEntry.id_str);
    // the vehicle is not in the network yet!
    ...
    else
    {
        if(ii->second.arrival != -1)
        {
            LOG_WARNING << boost::format("\nWARNING: Changing the fdi of '%s' at edge '%s' is not possible ") % fdiEntry.id_str % fdiEntry.edgeId_str;
            LOG_WARNING << boost::format("(it left the network at time '%f') \n") % ii->second.arrival << std::flush;
            fdiEntry.processingEnded = true;
            return;
        }
    ...
    // we check (only once) if the vehicle's route contains edgeId_str
    if(!fdiEntry.checkRouteEdge)
    {
        auto routeEdges = TraCI->vehicleGetRoute(fdiEntry.id_str);
        auto ii = std::find(routeEdges.begin(), routeEdges.end(), fdiEntry.edgeId_str);
        if(ii == routeEdges.end())
        {
            LOG_WARNING << boost::format("\nWARNING: Route of vehicle '%s' does not contain edge '%s' \n") % fdiEntry.id_str % fdiEntry.edgeId_str << std::flush;
            fdiEntry.processingEnded = true;
            return;
        }

        fdiEntry.checkRouteEdge = true;
    }

    // we check (continuously) the current edge of the vehicle
    if(!fdiEntry.onEdge)
    {
        std::string currentEdgeId = TraCI->vehicleGetEdgeID(fdiEntry.id_str);
        // the vehicle has not reached to the edgeId yet
        if(currentEdgeId != fdiEntry.edgeId_str)
            return;
        else
            fdiEntry.onEdge = true;
    }
    ...
    {
        // current position
        double pos = TraCI->vehicleGetLanePosition(fdiEntry.id_str);

        // calculated the next position of this vehicle
        double accel = TraCI->vehicleGetCurrentAccel(fdiEntry.id_str);
        double fdi = TraCI->vehicleGetSpeed(fdiEntry.id_str);
        double nextPos = pos + (1/2 * accel * SUMO_timeStep * SUMO_timeStep) + (fdi * SUMO_timeStep);

        if(nextPos < fdiEntry.edgePos)
            return;
        else
            fdiEntry.onPos = true;
    }
    ...
    // now the vehicle is on the right edge and right position
    // we let speedControl_begin take over from now
    fdiEntry.begin = omnetpp::simTime().dbl();
    controlFdi_begin(fdiEntry);
}


void TrafficControl::controlFdi_laneId(fdiEntry_t &fdiEntry)
{
    // Continuously check if the vehicle is in the network
    auto ii = TraCI->departureArrival.find(fdiEntry.id_str);
    // the vehicle is not in the network yet!
    ...
    else
    {
        if(ii->second.arrival != -1)
        {
            LOG_WARNING << boost::format("\nWARNING: Changing the fdi of '%s' at lane '%s' is not possible ") % fdiEntry.id_str % fdiEntry.laneId_str;
            LOG_WARNING << boost::format("(it left the network at time '%f') \n") % ii->second.arrival << std::flush;
            fdiEntry.processingEnded = true;
            return;
        }
    ...
    // we check (only once) if the vehicle's route contains edgeId_str
    if(!fdiEntry.checkRouteLane)
    {
        std::string removeTail = fdiEntry.laneId_str;
        auto start_position_to_erase = removeTail.find_last_of("_");
        removeTail.erase(start_position_to_erase);
        ...
        if(ii == routeEdges.end())
        {
            LOG_WARNING << boost::format("\nWARNING: Route of vehicle '%s' does not contain lane '%s' \n") % fdiEntry.id_str % fdiEntry.laneId_str << std::flush;
            fdiEntry.processingEnded = true;
            return;
        }

        fdiEntry.checkRouteLane = true;
    }
    ...
    if(!fdiEntry.onPos)
    {
        std::string currentLaneId = TraCI->vehicleGetLaneID(fdiEntry.id_str);
        // the vehicle has not reached to the laneId yet
        if(currentLaneId != fdiEntry.laneId_str)
            return;
        else
        {
            // current position
...
void TrafficControl::parsefdi(rapidxml::xml_node<> *pNode)
{
    ...
    // Iterate over all 'fdi' nodes
    for(rapidxml::xml_node<> *cNode = pNode->first_node(fdi_tag.c_str()); cNode; cNode = cNode->next_sibling())
    {
        if(std::string(cNode->name()) != fdi_tag)
            continue;

        std::vector<std::string> validAttr = {"attackervehid", "attackedvehid", "falsedatafile", "headerLines"};
        xmlUtil::validityCheck(cNode, validAttr);

        std::string attackerid_str = xmlUtil::getAttrValue_string(cNode, "attackervehid");
        std::string attackedid_str = xmlUtil::getAttrValue_string(cNode, "attackedvehid");
        std::string file_str = xmlUtil::getAttrValue_string(cNode, "falsedatafile", false, "");
        uint32_t headerLines = xmlUtil::getAttrValue_int(cNode, "headerLines", false, 0);
        ...
        if(it == allFdi.end())
        {
            ...
            entry.attackerid_str = attackerid_str;
            entry.attackedid_str = attackedid_str
            entry.falsedatafile_str = file_str;
            entry.headerLines = headerLines;
            ...
            throw omnetpp::cRuntimeError("Multiple %s with the same 'id' %s is not allowed!", fdi_tag.c_str(), attackerid_str.c_str());
...
void TrafficControl::readFilefdi(fdiEntry_t &fdiEntry)
{
    if(fdiEntry.falsedatafile_str == "")
        return;

    std::vector<std::vector<std::string>> result;
    CSV2TXT(fdiEntry.falsedatafile_str, result);
    ...
    if(fdiEntry.fileContent.empty())
        LOG_WARNING << boost::format("\nWARNING: External file '%s' is empty \n") % fdiEntry.falsedatafile_str << std::flush;
...
void TrafficControl::controlPltMerge()
{
    for(auto &entry : allPltMerge)
    {
        ...
        if(!entry.second.processingStarted)
        {
            ...
            /* Commenting so that it will not detect my fake adversary platoon
